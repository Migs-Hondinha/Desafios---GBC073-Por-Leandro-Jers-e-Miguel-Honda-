...
class Submissao:
    ...
    Miguel Augusto Cesário Honda - 12321BCC010
    '''
    DIM_MAX = DIM_MAX
    ...
    def fit(self, X: torch.Tensor) -> None:
        ...
        norma = torch.norm(((X - self.mu) / self.sd), dim=1).mean().item()

        gen = torch.Generator().manual_seed(0)
        self.W = torch.randn(X.shape[1], DIM_MAX, generator=gen) / math.sqrt(norma * X.shape[1]) 
        self.b = torch.rand(DIM_MAX, generator=gen)

    def phi(self, X: torch.Tensor) -> torch.Tensor:
        Xs = (X - self.mu) / self.sd
        return torch.relu(Xs @ self.W + self.b) 
